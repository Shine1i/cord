...
{#if userRune.authStore}
	...
	>
		<nav class=" h-full overflow-y-hidden">
			<ul role="list" class=" pt-4 flex relative min-h-full flex-col items-center space-y-1">
				<li>
					<button
						onclick={() => {
							goto('/calls');
				...
				{:then users}
					{#each users as user}
						<MessageFeed
							...
							messageTimeStamp={user.messageTimestamp?new Date(user.messageTimestamp).toLocaleDateString():''}
						/>
					{/each}
				{/await}
